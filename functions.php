<?php
/* Artifact setup and base functions.
 * Based on String Theory version 2.2 by TechnoSiren
 */
...
/* Custom Image Header */
	$args = array(
		...
		'flex-height'    => true,
		'height'        => 1016,
		'default-image' => get_template_directory_uri() . '/img/acropolis_bg.jpg',
	);
...
<?php if ( get_header_image() ) { ?>
	...
	<?php } else { ?>
	background-image: url('<?php echo get_template_directory_uri() ?>/img/acropolis_bg.jpg');
	<?php } ?>
...
function string_theory_font_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Montez font: on or off', 'stringtheory' ) ) {
		$font_url = add_query_arg( 'family', urlencode( 'Montez|Montserrat:400,700' ), "//fonts.googleapis.com/css" );
	}
...
function string_theory_scripts() {
	...
	wp_enqueue_style( 'string-theory-montez', string_theory_font_url(), array(), null );
...
?>	
	<script>
		// Based on Radial Menu Demo by http://creative-punch.net. Love it.
